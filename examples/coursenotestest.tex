...
\begin{matlab}[caption=First-Order System Simulation]
...
\begin{python}[caption=Python Control System Analysis]
...
where $\omega_{pc}$ is the phase crossover frequency ($\angle G(j\omega_{pc}) = -180^\circ$).

\begin{mmath}
\text{Phase Margin (PM)} \eq 180^\circ + \angle G(j\omega_{gc})
\end{mmath}
...
for phase margins between $0^\circ$ and $60^\circ$.
...
\begin{mmath}
\angle G(j\omega_{gc}) \eq -90^\circ - \arctan\left(\frac{\omega_{gc}}{10}\right) \\
\approxx -90^\circ - 16.7^\circ \\
\eq -106.7^\circ
\end{mmath}

Therefore, the phase margin is:
\begin{mmath}
\text{PM} \eq 180^\circ + \angle G(j\omega_{gc}) \\
\eq 180^\circ - 106.7^\circ \\
\eq 73.3^\circ
\end{mmath}
\end{solution}
...
\item Phase margin is positive ($73.3^\circ > 0^\circ$)
\item Phase margin exceeds the recommended minimum ($73.3^\circ > 30^\circ$)
\end{itemize}
\end{solution}
...
\item Two branches to infinity at angles $\pm 90^\circ$
...
\begin{solution}
For $\zeta = 0.707$, the closed-loop poles should be at $45^\circ$ angles from the real axis. From the root locus, this occurs at approximately $K = 2$.
...
Key takeaways:
\begin{enumerate}
\item Phase margin $>$ 45$^\circ$ ensures good transient response
\item Gain margin $>$ 6 dB provides robustness
