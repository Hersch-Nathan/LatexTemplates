...
\hwheader{COURSE 101}{Feature Demo}{2025-09-01}{Template Showcase}
...
% Feature 4: mmath Environment - Custom Math with Alignment and Comparison Operators
\problem{Demonstrate the mmath environment and new comparison shorthand operators.}

The mmath environment provides automatic alignment using shorthand commands:
...
% Feature 5: mathnumbered Environment - Numbered Equations
\problem{Show numbered equations for referencing.}

For equations that need to be referenced later, use mathnumbered:
...
\problem{Demonstrate the graphic command for including images.}

\subproblem
Command syntax and features:

The \texttt{\\graphic} command provides automatic centering with optional titles and scaling:

\texttt{\\graphic\{path\}[optional title][optional scale]}
...
\begin{itemize}
\item \texttt{\\graphic\{figure.pdf\}} — Basic inclusion with default width
\item \texttt{\\graphic\{figure.pdf\}[Plot Title]} — With title
\item \texttt{\\graphic\{figure.pdf\}[Title][0.6]} — With title and custom scale
\item \texttt{\\graphic\{figure.pdf\}[][0.5]} — Custom scale, no title
\end{itemize}
...
\begin{matlab}[caption=Second Order System Response Function]
...
\begin{python}[caption=Control Systems Analysis in Python]
import numpy as np
...
\begin{terminal}[caption=Package Installation and Testing]
...
\problem{Demonstrate the diagram environment and blox package features for control system diagrams.}

\subproblem
Classic PID feedback control system:

\begin{diagram}[scale=0.9]
\bXInput[r(s)]{R}
\bXComp{E}{R}
